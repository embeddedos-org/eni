...
eni_status_t eni_min_service_tick(eni_min_service_t *svc)
{
...
#ifdef ENI_HAS_DSP
    {
        ...
        eni_status_t dsp_st = eni_min_signal_processor_process(
            &svc->signal_processor, &norm_ev, &features_ev);
        if (dsp_st == ENI_OK) {
#ifdef ENI_HAS_DECODER
            eni_event_t decoded_ev;
            eni_status_t dec_st = eni_min_decoder_process(
