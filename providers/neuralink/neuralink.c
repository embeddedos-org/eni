...
static int nl_provider_init(void *ctx) { (void)ctx; return 0; }
static int nl_provider_read(void *ctx, void *buf, int len) {
    (void)ctx;
    if (len < (int)sizeof(eni_nl_packet_t)) return -1;
    return eni_neuralink_read_packet((eni_nl_packet_t *)buf);
}
static void nl_provider_deinit(void *ctx) { (void)ctx; eni_neuralink_deinit(); }

static const eni_provider_ops_t g_nl_provider = {
    .name = "neuralink",
    .init = nl_provider_init,
    .read = nl_provider_read,
    .deinit = nl_provider_deinit,
};
