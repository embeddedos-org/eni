...
static void test_provider_init(void)
{
    ...
    if (st != ENI_OK) { FAIL("provider init failed"); return; }
    if (strcmp(prov.name, "sim0") != 0) { FAIL("wrong provider name"); return; }
    PASS();
}
...
#include "providers/simulator/simulator.h"
...
static void test_simulator_ops_exist(void)
{
    ...
    if (!eni_provider_simulator_ops.init)    { FAIL("init is NULL"); return; }
    if (!eni_provider_simulator_ops.poll)    { FAIL("poll is NULL"); return; }
    if (!eni_provider_simulator_ops.shutdown) { FAIL("shutdown is NULL"); return; }
    PASS();
}
...
static void test_provider_init(void)
{
    ...
    eni_status_t st = eni_provider_init(&provider, &eni_provider_simulator_ops, "sim0", NULL);
    if (st != ENI_OK) { FAIL("provider init failed"); return; }
    if (strcmp(provider.name, "sim0") != 0) { FAIL("provider name mismatch"); return; }
    PASS();
}
